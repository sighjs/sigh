...
describe('write plugin', function () {
  ...
  it('write a binary file', function () {
    var data = new Buffer([0, 1, 2, 3, -1, 5, 6, 7, 0]);
    var stream = _sighCore.Bacon.constant([new _sighCore.Event({ basePath: 'subdir', path: PROJ_PATH_BINARY, type: 'add', data: data.toString('binary'), encoding: 'binary', supportsSourceMap: false })]);

    return (0, _pluginWrite2['default'])({ stream: stream }, TMP_PATH).toPromise(_bluebird2['default']).then(function (events) {
      // subdir stripped from the output path due to basePath
      var tmpFile = TMP_PATH + '/file2.bin';

      (0, _fs.readFileSync)(tmpFile).should.eql(data);
    });
  });
