...
exports['default'] = function (op) {
  ...
  var newEvent = function newEvent(type, _ref) {
    ...
    var props = { type: type, path: path, initPhase: initPhase, opTreeIndex: treeIndex, encoding: opts.encoding };
  ...
  var globOptions = _lodash2['default'].omit(opts, 'basePath');
  ...
  var makeGlobStream = function makeGlobStream(events) {
    var stream = _sighCore.Bacon.combineAsArray(patterns.map(function (pattern, idx) {
      return _sighCore.Bacon.fromPromise(glob(pattern, globOptions).then(function (paths) {
